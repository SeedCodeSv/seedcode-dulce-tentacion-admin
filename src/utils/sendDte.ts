...
export const uploadToS3 = async (key: string, body: Uint8Array<ArrayBufferLike>) => {
...
export const processSaleFCF = async (
  ...
  try {
    ...
    const uint8ArrayBuffer = encoder.encode(JSON_DTE)
    const uploadSuccess = await uploadToS3(json_url, uint8ArrayBuffer);
