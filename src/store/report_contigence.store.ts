...
import { get_contigence_not_report, get_contigence_report } from "../services/report_contigence.service";
...
export const useReportContigenceStore = create<ISalesContigenceStore>(
  (set) => ({
    ...
    OnGetSalesNotContigence: async (id: number, page: number, limit: number) => {
      const { data } = await get_contigence_not_report(id, page, limit);
      set({ saless: data.saless });
    },
  })
);
