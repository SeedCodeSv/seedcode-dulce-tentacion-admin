...
export const usePurchaseOrdersStore = create<PurchaseOrderStore>(
  (set, get) => ({
    ...
    updateOrderProduct(id, price, quantity) {
      const product = get().details_order_purchase.find(
        (cp) => cp.orderId === id
      );
      if (product) {
        set({
          details_order_purchase: get().details_order_purchase.map((cp) =>
            cp.orderId === id
              ? {
                ...
                total: typeof price === "number"
                  ? Number(price) * Number(cp.quantity)
                  : typeof quantity === "number" ? Number(quantity) * Number(cp.price) : cp.total,
              }
              : cp
          ),
        });
    ...
    deleteProductOrder(id) {
      ...
      if (find) {
        const products = get().details_order_purchase.filter(
          (cp) => cp.productId !== id
        );
    ...
    updatePriceOrder(id, price) {
      ...
      if (product) {
        set({
          details_order_purchase: get().details_order_purchase.map((cp) =>
            cp.productId === id ? { ...cp, price, cost: price * cp.quantity, total: price * cp.quantity } : cp
          ),
        });
      }
    },
    ...
    clearProductOrder() {
      const oldest_products = get().details_order_purchase.filter(
        (cp) => !cp.isNew
      );
      set({ details_order_purchase: oldest_products });
    },
    updatePurchaseOrder(id, details) {
      update_order(id, details).then(() => {
