...
} from '@/services/excluded_subject.service';
...
export const useExcludedSubjectStore = create<IExcludedSubjectStore>((set) => ({
  ...
  getExcludedSubjectByMonth(id, month, year) {
    get_excluded_subject_by_month(id, month, year)
      .then(({ data }) =>
        set({
          excluded_subjects_month: data.excludedSubject,
        })
      )
      .catch(() => {
        set({
