...
import { get_sales_by_branch_and_current_month, get_sales_by_day, get_sales_by_day_table, get_sales_by_month_and_year } from "../../services/reports/sales.reports.service";
...
export const salesReportStore = create<ISalesReportStore>((set) => ({
    ...
    sales_table_day: [],
    getSalesTableDay: (id) => {
        get_sales_by_day_table(id).then(({ data }) => {
    ...
    getSalesByYearAndMonth(id) {
        get_sales_by_month_and_year(id).then(({ data }) => {
