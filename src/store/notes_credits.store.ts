...
import { get_contingence_credit_notes, get_credit_notes, get_credit_notes_by_id, get_recent_credit_notes } from "@/services/credit_notes.service";
...
export const useCreditNotes = create<ICreditNotes>((set, get) => ({
  ...
  onGetCreditNotesPaginated(params) {
    set({loading: true})
    get_credit_notes(params).then(({ data }) => {
      set({ credit_notes_list: data, loading: false});
    })
      .catch(() => {
        set({
          credit_notes_list: {
