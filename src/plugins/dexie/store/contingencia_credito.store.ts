...
import { save_address_credito, get_address_credito } from '../services/credito_address.service';
...
import { add_credito_pagos, get_credito_pagos_por_id } from '../services/credito_pagos.service';
...
export const useContingenciaCreditoStore = create<IContingenciaCreditoStore>(() => ({
  ...
  async getCreditoVentaByCodigo(codigo) {
    const credito_venta = await get_credito_venta_by_codigo_generacion(codigo);

    if (credito_venta) {
      const credito_pagos = await get_credito_pagos_por_id(Number(credito_venta.id));
      const credito_receptor = await geT_credito_receptor_by_venta(Number(credito_venta.id));
      const credito_resumen = await get_credito_resumen_by_venta(Number(credito_venta.id));
      const credito_cuerpo_documento = await get_credito_cuerpo_documento_by_venta(
        Number(credito_venta.id)
      );
      const credito_address = await get_address_credito(Number(credito_receptor?.addressId));
