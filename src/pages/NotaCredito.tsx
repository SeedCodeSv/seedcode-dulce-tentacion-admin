                        ...
                        send_to_mh(data_send, token_mh ?? '', source)
                            .then(async (response) => {
                                ...
                                const json_url = `CLIENTES/${transmitter.nombre
                                    }/${new Date().getFullYear()}/VENTAS/NOTAS_DE_CREDITO/${formatDate()}/${nota_credito.dteJson.identificacion.codigoGeneracion
...
const handleVerify = () => {
  ...
  check_dte(payload, token_mh ?? '')
      ...
      .catch((error: AxiosError<ICheckResponse>) => {
          ...
          toast.error('ERROR', {
              description: `Error: ${error.response?.data.descripcionMsg ?? 'DTE no encontrado en hacienda'
                  }`,
          });
          setIsLoading(false);
      });
