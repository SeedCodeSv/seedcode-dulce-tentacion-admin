...
function ContingenceFSE() {
  ...
  const [startTime, setStartTime] = useState(
    contingence_excluded_subject[0]?.horEmi
  );
  // const box = 
  const [endDate, setEndDate] = useState(formatDate());
  const [endTime, setEndTime] = useState(
    contingence_excluded_subject[contingence_excluded_subject.length - 1]?.horEmi
  );
  ...
  useEffect(() => {
    onGetContingenceExcludedSubject(Number(branchId));
  ...
  const timeStart = useMemo(() => {
    if (contingence_excluded_subject.length > 0) {
      setStartTime(contingence_excluded_subject[0]?.horEmi);

      return contingence_excluded_subject[0]?.horEmi;
    }
  ...
  const timeEnd = useMemo(() => {
    if (contingence_excluded_subject.length > 0) {
      setEndTime(contingence_excluded_subject[contingence_excluded_subject.length - 1]?.horEmi);

      return contingence_excluded_subject[contingence_excluded_subject.length - 1]?.horEmi;
    }
