...
function GraphicProductCategory(props: Props) {
  ...
  const totalPages = Math.ceil(graphic_for_category_products_for_dates.length / itemsPerPage);
  ...
  }, [windowSize.width, graphic_for_category_products_for_dates, currentPage, itemsPerPage]);
  ...
  const [selectedSubCategory, setSelectedSubCategory] = useState<GraphicSubCategory>();

  return (
    <>
      <div className="flex items-center justify-between mt-6">
        <p className="pb-4 text-lg font-semibold dark:text-white">Ventas por categoría</p>
          ...
          chart: {
            events: {
              ...
              xAxisLabelClick(_, __, options) {
                const label_selected = options.config.labels[options.labelIndex];
          ...
          legend: {
            ...
            labels: {
              colors: [
                ...(context === 'light'
                  ? ['#000']
                  : [...Array.from({ length: labels.length }, () => '#fff')]),
              ],
            },
          },
          yaxis: {
            ...
            <div className="w-full p-5 mt-5 bg-white border shadow dark:bg-gray-950 rounded-2xl">
              {windowSize.width < 900 && selectedSubCategory && (
                <div className="flex flex-col items-center justify-center">
                  ...
                  <p className="py-2 text-sm font-semibold text-left dark:text-white">
                    Total: {formatCurrency(Number(selectedSubCategory?.totalItems) || 0)}
                  </p>
                </div>
              )}
              <ApexCharts
                    ...
                    events: {
                      dataPointSelection(_, __, options) {
                        setSelectedSubCategory(
                          graphic_sub_category_products_for_dates[options.dataPointIndex]
                        );
                      },
                    },
                  },
