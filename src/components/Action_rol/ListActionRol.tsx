...
const PermissionTable: React.FC = () => {
  ...
  const userPermissions = ['Mostrar', 'Agregar', 'Editar', 'Eliminar', 'Cambiar Contraseña'];
  ...
  const handleSelectAllActions = (viewId: number) => {
    const isUserView =
      viewasAction.find((view) => view.view.id === viewId)?.view.name === 'Usuarios';
    const allPermissions = isUserView ? userPermissions : permissions;

    setSelectedActions((prev) => {
      const actions = prev[viewId] || [];
      const allSelected = actions.length === allPermissions.length;
      const newActions = allSelected ? defaultActions[viewId] : allPermissions;
      return {
        ...prev,
        [viewId]: newActions,
      };
...
const renderActions = (viewId : number) => {
  ...
  const isUserView = view?.name === 'Usuarios';
  const currentPermissions = isUserView ? userPermissions : permissions;
    ...
    <div className="w-full p-2" key={viewId}>
      <div className="mb-4 bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden">
        <div className="px-4 py-3 bg-blue-900 dark:bg-blue-700 text-white flex justify-between items-center">
          ...
          <div
            className={`cursor-pointer ${selectedActions[viewId]?.length === currentPermissions.length ? 'bg-green-500' : 'bg-gray-500'} rounded-full h-8 w-8 flex items-center justify-center`}
          ...
          <div className="bg-white dark:bg-gray-800">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {currentPermissions.map((permission) => (
                <div
                  key={permission}
                  className={`flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-gray-700 cursor-pointer ${defaultActions[viewId]?.includes(permission) ? 'cursor-not-allowed' : ''}`}
                  onClick={() => handleSelectAction(viewId, permission)}
                >
                  <span className="dark:text-white">{permission}</span>
                  <div
                    className={`${selectedActions[viewId]?.includes(permission) ? 'bg-green-500' : 'bg-gray-500'} rounded-full h-8 w-8 flex items-center justify-center`}
                  >
                    <Check className="text-white" />
                  </div>
                </div>
      ...
      <div className="overflow-y-auto h-64 lg:h-full">
        {viewasAction.map((view) => (
          <div key={view.view.id} className="mb-2">
            <button
              className={`w-full text-left bg-white dark:bg-gray-800 py-2 px-4 rounded-lg shadow-md hover:bg-gray-200 dark:hover:bg-gray-700 transition ${selectedViewId === view.view.id ? 'bg-blue-200 dark:bg-blue-700' : ''}`}
      ...
      <div className="w-full h-full p-5 overflow-y-auto bg-white dark:bg-gray-900 shadow-xl rounded-xl">
        <div className="flex flex-col lg:flex-row items-center justify-between w-full gap-5 mb-5 lg:mb-10 lg:gap-0">
          <div className="flex items-center gap-3">
            <h1 className="hidden lg:block dark:text-white">Agregar Acciones a Vistas</h1>
          </div>
