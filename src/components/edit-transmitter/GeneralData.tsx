...
function GeneralData() {
    ...
    const { fiscalDataAndParameter, getFiscalDataAndParameter, onCreateFiscalDataAndParameter, onUpdateFiscalDataAndParameter } = useFiscalDataAndParameterStore()

    useEffect(() => {
        if (user) {
            const transId = user.correlative ? user.correlative.branch.transmitter.id : user.pointOfSale ? user.pointOfSale.branch.transmitter.id : 0
        ...
        onSubmit(values, formikHelpers) {
            if (user) {
                ...
                if (fiscalDataAndParameter) {
                    onUpdateFiscalDataAndParameter(fiscalDataAndParameter.id, { ...values, transmitterId: transId })
        ...
        <>
            <form onSubmit={(e) => {
                ...
                <div className='w-full flex justify-end mt-3 gap-5'>
                    <Button onClick={() => navigation("/configuration")} isLoading={formik.isSubmitting} className='px-20 font-semibold' style={styles.dangerStyles} type='submit'>Cacelar</Button>
                    <Button isLoading={formik.isSubmitting} className='px-20 font-semibold' style={styles.thirdStyle} type='submit'>Guardar</Button>
                </div>
            </form>
        </>
    );
