...
const DetailItem: React.FC<DetailItemProps> = ({ label, value }) => (
  <div className="flex flex-col py-4 border-b border-gray-100">
    <span className="text-gray-500 dark:text-gray-300/80 text-sm">{label}</span>
    ...
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow p-4 mb-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
        ...
        <div className="flex flex-col py-4 col-span-1 md:col-span-2 border-b border-gray-100">
          <span className="text-gray-500 dark:text-gray-300/80 text-sm">Observaciones</span>
        <ul className="list-disc pl-5 space-y-1">
          {observations.map((observation, index) => (
            <li key={index} className="text-gray-800 dark:text-gray-100 font-medium">
              {observation}
            </li>
          ))}
        </ul>
