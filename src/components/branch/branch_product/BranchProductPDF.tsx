...
const DownloadPDFButton = ({  branch, transmitter }: {  branch: Branches,  transmitter: ITransmitter,}) => {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      await getBranchProductsFilteredList({branchId: branch.id})

        if (!branchProductsFilteredList || branchProductsFilteredList.length === 0) {
      ...
      // const rows = branchProductsFilteredList.map((item, index) => [
      //   item.id,
      //   `${item.date} - ${item.time}`,
