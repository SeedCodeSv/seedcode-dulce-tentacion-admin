...
import { MenuDetail, ProductMenu } from '@/types/menu.types';

type ProductOrder = ProductMenu & { menuDetailId: number, quantity: number; uniMedidaExtra: string, isActive: boolean };

interface Props {
    menuDetails: MenuDetail[];
    setSelecteMenu: (products: ProductOrder[]) => void;
}

function UpdateRecipeBook({ menuDetails, setSelecteMenu }: Props) {
    const [includesPrescription, setIncludesPrescription] = useState(true);
    ...
    useEffect(() => {
        if (menuDetails?.length > 0) {
            const initialProducts: ProductOrder[] = menuDetails.map((md) => ({
                ...md.branchProduct.product,
                id: md.branchProduct.product.id,
                menuDetailId: md.id,
                quantity: Number(md.quantity),
                uniMedidaExtra: md.uniMedia,
                branchProductId: md.branchProductId,
            }));

            setSelectedProducts(initialProducts);
            setSelecteMenu(initialProducts)
            setIncludesPrescription(true)
        } else {
            setSelectedProducts([]);
            setSelecteMenu([])
        }
    ...
    const handleAddSupplier = (prd: Product) => {
        ...
        const index = list_suppliers.findIndex((lsP) => lsP.id === prd.id);

        if (index === -1) {
            const uniMedidaExtra = prd.uniMedida ?? 'default';
            const product: ProductOrder = {
                id: prd.id,
                menuDetailId: 0,
                name: prd.name,
                description: prd.description,
                tipoItem: prd.tipoItem,
                tipoDeItem: prd.tipoDeItem,
                uniMedida: prd.uniMedida,
            ...
            list_suppliers.push(product);
        } else {

            list_suppliers[index].isActive = !list_suppliers[index].isActive;
        }

        setSelectedProducts(list_suppliers);
        setSelecteMenu(list_suppliers);
    };


    const checkIsSelectedSupplier = (id: number) => {
        return selectedProducts.some((ssp) => ssp.id === id && ssp.isActive);
    };
    ...
    const handleDeleteProduct = (index: number) => {
        ...
        list_suppliers[index].isActive = false;
            ...
            <div className="shadow border rounded-[12px] p-5">
                <div className="flex justify-between">
                    <Checkbox
                        ...
                        isSelected={includesPrescription}
                        size="lg"
                ...
                <div className="w-full mt-3 max-h-96 overflow-y-auto md:grid md:grid-cols-3 flex flex-col gap-4">
                    {selectedProducts.filter((sp) => sp.isActive).map((sp: ProductOrder) => (
                        <div
                            key={sp.id}
