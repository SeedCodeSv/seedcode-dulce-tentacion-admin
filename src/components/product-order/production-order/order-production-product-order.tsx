...
export default function OrderProductionProductOrder() {
  const { selectedProducts, handleVerifyProduct, errors, verified_product } =
  ...
  const OnVerifyProduct = async (id: number, quantity: string) => {
    ...
    if (
      ...
      res.message?.includes(
        "TypeError: Cannot read properties of null (reading 'productRecipeBookDetails')"
      )
    ) {
  ...
  const handleAddProductRecipe = async (
    recipe: ResponseVerifyProduct,
    quantity?: string
  ): Promise<void> => {
        ...
        <DivGlobal className="flex h-full">
                          ...
                          <tr key={item.id}>
                            ...
                            <TdGlobal>
                              <Button
                                ...
                                variant="bordered"
                                onPress={() =>
                                  OnVerifyProduct(item.product.id, item.quantity ?? '')
                                }
                              >
          ...
          <Modal {...modalError}>
            <ModalContent>
              ...
              <ModalBody>
                <strong>{`Problemas con los insumos del producto seleccionado: `}</strong>
                {errors &&
                  errors.length > 0 &&
                ...
                <ButtonUi
                  ...
                  onPress={() => {
                    if (errors.some((item) => item.exist === false)) {
                      toast.error(
                        'Algunos productos no existen en la sucursal de origen. Verifica tu selección.',
                        { duration: 7000 }
                      );
