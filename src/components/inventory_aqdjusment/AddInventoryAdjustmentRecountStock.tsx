...
export default function AddInventoryAdjustmentRecountStock() {
  ...
  const { OnCreateRecountStockInventoryAdjustment } = useIInventoryAdjustmentStore();
  ...
  const handleCreate = async () => {
    const detailInventoryAdjustments = card_products.map((product) => ({
      ...
      remainingPrice: productData[product.id]?.remainingPrice ?? product.costoUnitario,
    }));

    const ok = await OnCreateRecountStockInventoryAdjustment({
      description: dataCreate.description,
      branchId: branch.id,
      userId: userId ?? 0,
      detailInventoryAdjustments,
      typeMovement: 'Entradas',
      typeInventory: 'Recuento',
    });
              ...
              <div className="flex sm:w-full w-1/2">
                <Autocomplete
                  ...
                  onSelectionChange={(key) => {
                    const selectedBranch = branch_list.find((branch) => branch.id === Number(key));
                      ...
                      <table className="w-full">
                        <thead className="sticky top-0 z-[999] bg-white dark:bg-slate-800">
                          <tr>
                            ...
                            <th
                              className="p-3 text-sm font-semibold text-left whitespace-nowrap text-slate-600 dark:text-gray-100 bg-slate-200 dark:bg-slate-700"
                              style={global_styles().darkStyle}
                            >
                              Costo
                            </th>
                          ...
                          <AnimatePresence>
                            {card_products.map((article) => (
                              ...
                              >
                                ...
                                <td className="p-3 text-sm  text-slate-500 whitespace-nowrap dark:text-slate-300">
                                  {(productData[article.id]?.remainingStock ?? 0) - article.stock}
                                </td>
                      ...
                      >
                        <div className="p-4 space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-base">
                            ...
                            <div className="space-y-3">
                              <div className="flex justify-between items-center">
                                <span className="text-gray-500 dark:text-white">Stock Final:</span>
                                <span className="font-semibold text-gray-900 dark:text-white">
                                  {dataCreate.typeMovement === 'Entradas'
                                    ? article.stock + (productData[article.id]?.remainingStock ?? 0)
                                    : article.stock -
