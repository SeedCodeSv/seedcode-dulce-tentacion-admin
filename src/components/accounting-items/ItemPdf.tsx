...
function ItemPdf({ JSONData, date, correlative }: Props) {
  ...
  function consolidateAccounts(data: Data[][]) {
    ...
    const allAccounts = [...consolidatedArray, ...intermediateArray, ...finalElements];
    ...
    allAccounts.forEach((account) => {
      const mainAccountCode = account.account.length === 4 ? account.account : account.majorAccount;
  ...
  const exportToPdf = () => {
    ...
    const dataPlaned = consolidateAccounts(JSONData as unknown as Data[][]).flat(1);
    ...
    autoTable(doc, {
      ...
      didDrawCell: (data) => {
        ...
        if (data.column.index === 2) {
          const accountValue = String(data.row.raw ? (data.row.raw as string[])[0] : '');
    ...
    autoTable(doc, {
      ...
      body: [['', '', 'Totales:', formatMoney($totalDebe), formatMoney($totalHaber)]],
      theme: 'plain',
      columnStyles: {
        0: {
          cellWidth: 25,
      ...
      didDrawCell: function (data) {
        if (data.column.index >= 3) {
          doc.setDrawColor(0, 0, 0);
          doc.setLineWidth(0.3);
          doc.line(data.cell.x + 4, data.cell.y, data.cell.x + data.cell.width, data.cell.y);
    ...
    autoTable(doc, {
      ...
      didDrawCell(data) {
        if (data.column.index === 0) {
          doc.setDrawColor(0, 0, 0);
          doc.setLineWidth(0.3);
          doc.line(data.cell.x + 4, data.cell.y, data.cell.x + data.cell.width - 30, data.cell.y);
          doc.text('Hecho por:', data.cell.x + 25, data.cell.y + 5);
        }
        if (data.column.index === 1) {
          doc.setDrawColor(0, 0, 0);
          doc.setLineWidth(0.3);
          doc.line(data.cell.x + 10, data.cell.y, data.cell.x + data.cell.width - 20, data.cell.y);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`${completeDateFormat(date)}`, doc.internal.pageSize.getWidth() / 2, 25, {
        align: 'center',
      });
      doc.setFont('helvetica', 'bold');
