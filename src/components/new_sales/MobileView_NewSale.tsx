...
function MobileView_NewSale({ layout }: Props) {
  const { branch_products, getPaginatedBranchProducts } = useBranchProductStore();
    ...
    <div className="w-full">
      <DataView
        ...
        pt={{
          grid: () => ({
            className:
              'grid dark:bg-slate-900 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 xl:grid-cols-4 gap-5 mt-5',
          }),
        }}
        color="surface"
